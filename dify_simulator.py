...
def resolve_bs_org_rates(primary_dept: str, allocations: Optional[List[Dict[str, Any]]]):
...
def calculate_profitability_ccs(total_price: int, cogs: int, direct_labor_cost: int, sga_rate_on_labor: float, target_margin_input: Optional[float]):
...
def main(**kwargs):
    ...
    try:
        ...
        return {"calc_json": json.dumps(data, ensure_ascii=False, indent=2)}
...
if __name__ == "__main__":
    # Difyで設定されている入力変数を想定したダミーデータ
    mock_inputs = {
        "complexity": "medium",
        "screen_count": 10,
        "features": "認証・認可 (Auth/SSO), 管理画面 (Admin)",
        "confidence": "high",
        "phase2_items": "基本設計書作成",
        "phase3_items": "",
        "duration": "normal",
        "dev_type": "new",
    ...
    print("=== Dify Code Node Simulator ===")
    print("Inputs:", json.dumps(mock_inputs, ensure_ascii=False, indent=2))
    
    result = main(**mock_inputs)
    
    print("\n=== OUTPUT (calc_json) ===")
    print(result.get("calc_json"))
