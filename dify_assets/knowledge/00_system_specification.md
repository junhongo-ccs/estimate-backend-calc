...
|F-003|RAG（検索）|Azure AI Search から過去の類似案件や基準を検索|
|F-004|根拠HTML生成|Azure OpenAI による説明文生成（RAG結果を活用）|
|F-005|統合レスポンス|金額・根拠・前提・注意事項を返却|
...
└──────────────┬──────────────┬──────────────┘
               ...
               ▼              ▼
┌──────────────────────┐    ┌──────────────────────┐
│ estimate-backend-calc│    │ Azure AI Search      │
│ - Azure Functions    │    │ - 過去案件データ     │
│ - YAML係数設定       │    │ - 見積基準ドキュメント│
└──────────────────────┘    └──────────────────────┘
```
...
|**RAG**|**Azure AI Search**|**Azureに格納（検索用）**|
...
|Azure AI Search|RAG 用の検索エンジン（Azure に格納）|
